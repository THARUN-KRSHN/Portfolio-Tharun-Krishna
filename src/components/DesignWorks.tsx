        ...
        <section ref={targetRef} className="relative h-[300vh] bg-background">
            <div className="sticky top-0 flex h-screen items-center overflow-hidden">
                <div className="px-20 mb-auto mt-32 absolute left-0 z-10">
                    <h3 className="text-4xl md:text-6xl font-display font-medium text-foreground dark:mix-blend-difference">
                        Visual <br /> Narratives
                    </h3>
                    <p className="mt-4 text-muted-foreground max-w-xs font-medium">
                ...
                <motion.div style={{ x }} className="flex gap-12 pl-[40vw]">
                    {posters.map((project, index) => (
                        <div
                            key={project.id}
                            className="relative group h-[60vh] aspect-[3/4] flex-shrink-0 bg-secondary rounded-none overflow-hidden perspective-1000"
                        >
                            <motion.div
                            ...
                            >
                                ...
                                <div className="absolute bottom-0 left-0 p-6 bg-gradient-to-t from-black/80 to-transparent w-full opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                                    ...
                                    <div className="text-muted-foreground text-sm">{project.tags.join(" • ")}</div>
                                </div>
                            </motion.div>
                        </div>
                    ))}
                    <div className="h-[60vh] w-[20vw] flex items-center justify-center text-muted-foreground font-display text-9xl opacity-20">
                        END
                    </div>
                </motion.div>
            </div>
            ...
            {social.length > 0 && (
                <div>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {social.map(p => (
                            <div key={p.id} className="aspect-[4/5] bg-secondary overflow-hidden relative group">
